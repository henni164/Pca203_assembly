...
pt_pgt_plot <- ggplot() +
  geom_segment(data = just_pgt_pt_cis, aes(x = num.x, xend = num.y, y = new_bpcum.x, yend = new_bpcum.y), size = 0.05) +
  geom_segment(data = just_pgt_pt_trans, aes(x = num.x, xend = num.y, y = new_bpcum.x, yend = new_bpcum.y), size = 0.05) +
  geom_rect(data = plotting_dat2, aes(xmin = xstart, xmax = xend, ymin = ystart, ymax = yend, fill = color)) +
  geom_segment(data = plotting_dat2, aes(x = xstart, xend = xend, y = centro, yend = centro), color = "red") + 
  scale_fill_manual(values = c("#619CFF", "#F8766D")) +
  scale_x_continuous(breaks = c(1,3), labels = c("Pgt21-0", "PtLR1")) +
  labs(x = "Genome", y = NULL) +
  theme(axis.line.x = element_blank(),
        axis.line.y = element_blank(),
        panel.grid = element_blank(),
        axis.text.y = element_blank(),
...
ggsave("/Users/evahenningsen/Documents/Research/203_assembly/synteny_plot_pt_pgt.jpg", plot = pt_pgt_plot, device = "jpeg", width = 10, height = 10, units = "in")

pca_pgt_plot <- ggplot() +
  geom_segment(data = just_pca_pgt_cis, aes(x = num.x, xend = num.y, y = new_bpcum.x, yend = new_bpcum.y), size = 0.05) +
  geom_segment(data = just_pca_pgt_trans, aes(x = num.x, xend = num.y, y = new_bpcum.x, yend = new_bpcum.y), size = 0.05) +
  geom_rect(data = plotting_dat3, aes(xmin = xstart, xmax = xend, ymin = ystart, ymax = yend, fill = color)) +
  geom_segment(data = plotting_dat3, aes(x = xstart, xend = xend, y = centro, yend = centro), color = "red") + 
  scale_fill_manual(values = c("#619CFF", "#00BA38")) +
  scale_x_continuous(breaks = c(1,2), labels = c("Pgt21-0", "Pca203")) +
  labs(x = "Chromosome", y = NULL) +
  ggtitle("Chromosome 8A") +
  theme(axis.line.x = element_blank(),
        axis.line.y = element_blank(),
        panel.grid = element_blank(),
        axis.text.y = element_blank(),
...
ggsave("/Users/evahenningsen/Documents/Research/203_assembly/synteny_plot_pca_pgt.jpg", plot = pca_pgt_plot, device = "jpeg", width = 3, height = 6, units = "in")


pca_pt_plot <- ggplot() +
  geom_segment(data = just_pca_pt_cis, aes(x = num.x, xend = num.y, y = new_bpcum.x, yend = new_bpcum.y), size = 0.05) +
  geom_segment(data = just_pca_pt_trans, aes(x = num.x, xend = num.y, y = new_bpcum.x, yend = new_bpcum.y), size = 0.05) +
  geom_rect(data = plotting_dat4, aes(xmin = xstart, xmax = xend, ymin = ystart, ymax = yend, fill = color)) +
  geom_segment(data = plotting_dat4, aes(x = xstart, xend = xend, y = centro, yend = centro), color = "red") + 
  scale_fill_manual(values = c("#00BA38", "#F8766D")) +
  scale_x_continuous(breaks = c(2,3), labels = c("Pca203", "PtLR1")) +
  labs(x = "Chromosome", y = NULL) +
  ggtitle("Chromosome 8A") +
  theme(axis.line.x = element_blank(),
        axis.line.y = element_blank(),
...
ggsave("/Users/evahenningsen/Documents/Research/203_assembly/synteny_plot_pca_pt.jpg", plot = pca_pt_plot, device = "jpeg", width = 3, height = 6, unit = "in")
